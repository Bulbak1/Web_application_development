...
def _get_app_module():
    for name in ('__main__', 'app'):
        ...
        if mod is not None and hasattr(mod, 'db'):
            return mod
    raise RuntimeError('Не удалось найти модуль приложения')
...
def pages_report():
    ...
    return render_template('visit_logs/pages.html', title='Отчёт по страницам', stats=stats)
...
def users_report():
    ...
    return render_template('visit_logs/users.html', title='Отчёт по пользователям', stats=stats)
